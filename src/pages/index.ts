export { default as Page404 } from './Page404';
export { default as HomePage } from './DashboardPage';
export { default as LendPage } from './LendPage';
export { default as StrategiesPages } from './StrategiesPages';
export { default as MyStrategiesPage } from './StrategiesPages/MyStrategiesPage';
export { default as StrategyCreationPage } from './StrategiesPages/StrategyCreationPage';
export { default as LiquidationsPage } from './LiquidationsPage';
export { default as LoansPage } from './LoansPage';
export { default as MarketsPage } from './MarketsPage';

export {
  BorrowRootPage,
  BorrowManualPage,
  BorrowBulkSuggestionPage,
  BorrowBulkOverviewPage,
  BorrowSuccessPage,
} from './BorrowPages';
export { OfferPage } from './OfferPage';
