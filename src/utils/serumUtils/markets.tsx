...
import { useAccountData, useAccountInfo } from './connection';
...
export function useUnmigratedOpenOrdersAccounts() {
  ...
  async function getUnmigratedOpenOrdersAccounts(): Promise<OpenOrders[]> {
    if (!wallet || !connection || !walletPublicKey) {
      return [];
    }
...
export function useOpenOrdersAccounts(fast = false) {
  const { market } = useMarket();
  const { connected, wallet, publicKey: walletPublicKey } = useWallet();
  const { connection } = useConnection();
...
export function useTokenAccounts(): [
  ...
  async function getTokenAccounts() {
    ...
    return await getTokenAccountInfo(connection, walletPublicKey);
  }
  return useAsyncData(
    getTokenAccounts,
...
export function useAllOpenOrdersAccounts() {
  const { wallet, connected, publicKey: walletPublicKey } = useWallet();
  const { connection } = useConnection();
    ...
    return (
      await Promise.all(
        programIds.map((programId) =>
          OpenOrders.findForOwner(connection, walletPublicKey, programId),
        ),
      )
    ).flat();
  };
  ...
  async function getOpenOrdersForDeprecatedMarkets() {
    ...
    const getOrders = async (market: Market | null) => {
      ...
      try {
        ...
        return (
          await market.loadOrdersForOwner(connection, walletPublicKey)
